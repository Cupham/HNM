...
import echowand.services.DeviceObserve.FaultType;
	...
	public static String getFaultDetail(ObjectData odata) throws EchonetObjectException {
		...
		if (allData[1] != (byte) 0x00) {
			...
			case (byte) 0x6E:
				...
				break;
			default:
				if (ConvertData.dataToInteger(allData) <= (byte) 0x03E8
						&& ConvertData.dataToInteger(allData) >= (byte) 0x006F) {
					contentSpecification = "User-definable domain";
					faultType = FaultType.Require_Repair_Faults;
				} else {
					faultType = FaultType.Undefined;
	...
	public static byte[] requestData(EPC epc, String data) {
		try {
			...
			case x80:
				...
				if (status == null)
					throw new ParseException("Cannot parse to boolean as operation status: EPC.x80.");
				result = new byte[1];
			...
			case x87:
				...
				if (setting == null) {
					throw new ParseException("Cannot parse to number as current limit setting: EPC.x87.");
				}
				result = new byte[1];
			...
			case x8F:
				...
				if (saving == null) {
					throw new ParseException("Cannot parse to boolean as power-saving operation setting: EPC.x8F.");
				}
				result = new byte[1];
			...
			case x93:
				...
				if (remote == null) {
					throw new ParseException("Cannot parse to boolean as remote control setting: EPC.x93.");
				}
				result = new byte[1];
			...
			case x97:
				...
				if (hour == null || minutes == null) {
					throw new ParseException("Cannot parse to time as current time setting: EPC.x97.");
				}
				result = new byte[2];
			...
			case x98:
				...
				if (year == null || month == null || day == null) {
					throw new ParseException("Cannot parse to time as current date setting: EPC.x98.");
				}
			...
			case x99:
				...
				if (power == null) {
					throw new ParseException("Cannot parse to number as power limit setting: EPC.x99.");
				}
